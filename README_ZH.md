...
UDPlex 是一款高性能、组件化的 UDP 流量复制、转发、过滤与智能路由工具：支持多目标并行分发与回程处理，并提供鉴权加密、协议识别、负载均衡以及多线路冗余/按带宽阈值智能分流（可用于链路加速与带宽叠加）等能力。
...
当前 `udplex-wg-manager.sh` 已切换为 UDPlex 内嵌 `wireguard-go` 的部署方式，不再依赖系统 `wg-quick`。
它支持：

- 运行时自动创建 WireGuard 网卡
- 两条外层线路分别独立选择协议，支持 `UDP+UDP`、`UDP+TCP`、`TCP+UDP`、`TCP+TCP`
- 低流量时双线路冗余发送
- 高流量时可选两种策略：
  按包序号在两条线路间均衡分流，或固定走一条首选线路
- 让 WireGuard 数据继续通过 `forward` 或 `tcp_tunnel_*` 组件进行转发
...
- **负载均衡**：根据流量大小和负载策略智能分发数据包。比如流量较小时可同时向两条线路发送做冗余；超过阈值后，可以选择在两条线路之间均衡分流，或固定走一条首选线路。
...
- [**wg_component_load_balancer_client.yaml**](examples/wg_component_load_balancer_client.yaml) - 内嵌 WireGuard + 双线路负载均衡客户端配置
