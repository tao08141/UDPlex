...
func (l *ListenComponent) handlePackets() {
    ...
    for {
        ...
        default:
            ...
            // Copy the data to avoid races if buffer is reused
            // data := make([]byte, length)
            // copy(data, buffer[:length])
            
            // Forward the packet to detour components
            if err := l.router.Route(buffer[:length], addr, l.tag, l.detour); err != nil {
...
func (l *ListenComponent) HandlePacket(data []byte, srcAddr net.Addr, srcTag string) error {
