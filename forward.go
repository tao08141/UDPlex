        ...
        case <-ticker.C:
            for _, conn := range f.forwardConnList {
                if atomic.LoadInt32(&conn.isConnected) == 0 {
                    ...
                    // Send keepalive
                    select {
                    case conn.sendQueue <- []byte{0}: // Empty packet as keepalive
                    default:
...
func (f *ForwardComponent) setupForwarder(remoteAddr string) (*ForwardConn, error) {
    ...
    forwardConn := &ForwardConn{
        conn:                 conn,
        sendQueue:            make(chan []byte, f.queueSize),
        isConnected:          1,
    ...
    for atomic.LoadInt32(&conn.isConnected) == 1 {
        ...
        default:
            ...
            if length > 0 && length != 1 { // Skip empty keepalive packets
                // Copy the data to avoid races
                data := make([]byte, length)
                copy(data, buffer[:length])
                
                // Forward to detour components
                if err := f.router.Route(data, nil, f.tag, f.detour); err != nil {
...
func (f *ForwardComponent) HandlePacket(data []byte, srcAddr net.Addr, srcTag string) error {
